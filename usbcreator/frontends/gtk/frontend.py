...
ui_path = str(here.parent.parent / "gui/usbcreator-gtk.ui")
...
class GtkFrontend(Frontend):
    ...
    def add_source(self, source):
        ...
        _append_to_list_and_select(self.source_treeview, [source], force_selection=True)
    ...
    def add_target(self, target):
        ...
        _append_to_list_and_select(
            self.dest_treeview, [str(target)], force_selection=False
        )
    ...
    def setup_sources_treeview(self):
        ...
        self.source_treeview.drag_dest_set(
            Gtk.DestDefaults.ALL,
            [Gtk.TargetEntry.new("text/uri-list", 0, 600)],
            Gdk.DragAction.COPY,
        )
        ...
        if source and target:
            ...
            try:
                self.backend.install(
                    source, target, allow_system_internal=self.allow_system_internal
                )
            except:
                self._fail()
    ...
    def retry(self, message):
        retry_dialog = Gtk.MessageDialog(
            transient_for=self.window,
            modal=True,
    ...
    def success(self):
        ...
        try:
            ...
            bus = dbus.SystemBus()
            obj = bus.get_object("com.ubuntu.USBCreator", "/com/ubuntu/USBCreator")
            if obj.KVMOk(
                dbus_interface="com.ubuntu.USBCreator", timeout=misc.MAX_DBUS_TIMEOUT
            ):
                self.kvm_test.show()
