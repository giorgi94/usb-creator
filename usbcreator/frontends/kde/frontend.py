...
ui_path = str(here.parent.parent / "gui/usbcreator-kde.ui")
...
class KdeFrontend(Frontend, QObject):
    ...
    def __initUI(self):
        ...
        #         QStandardPaths.DataLocation, self.__mainWindow_ui
        #     )
        #     appdir = file[: file.rfind("/")]
        # uic.loadUi(
        #     misc.text_type(appdir + "/" + self.__mainWindow_ui), self.__mainWindow
        # )
        ...
        self.__mainWindow.ui_dest_list.currentItemChanged.connect(
            self.dest_selection_changed
        )
        self.__mainWindow.ui_source_list.currentItemChanged.connect(
        ...
        self.__mainWindow.ui_source_list.header().setSectionResizeMode(
        ...
        self.__mainWindow.ui_dest_list.header().setSectionResizeMode(
            0, QHeaderView.Stretch
        )
        self.__mainWindow.ui_dest_list.header().setSectionResizeMode(
    ...
    def add_source(self, source):
        ...
        if self.__backend is not None:
            ...
            new_item.setToolTip(1, new_item.text(1))
            new_item.setText(
                2, misc.format_size(self.__backend.sources[source]["size"])
            )
            new_item.setToolTip(2, new_item.text(2))
        ...
        if (
            self.__recently_added_image != None
            and self.__backend.sources[source]["device"] == self.__recently_added_image
        ):
    ...
    def add_file_source_dialog(self):
        ...
        downloadPath = QStandardPaths.standardLocations(
            QStandardPaths.DownloadLocation
        )[0]
        openFileName = QFileDialog.getOpenFileName(
            self.__mainWindow, "", downloadPath, filter
        )
        if openFileName:
    ...
    def install(self):
        ...
        if source and target:
            ...
            msgbox.setText(
                _("Are you sure you want to write the disc image to the device?")
            )
            ...
            try:
                self.__backend.install(
                    source, target, allow_system_internal=self.__allow_system_internal
                )
            except:
                self.__fail()
        else:
    ...
    def success(self):
        ...
        self.progress_bar.hide()
        text = _(
            "The installation is complete.  You may now reboot your "
    ...
    def retry(self, message):
        ...
        res = QMessageBox.warning(
            self.__mainWindow, caption, message, QMessageBox.Yes, QMessageBox.No
        )
    ...
    def warning_dialog(self):
        ...
        res = QMessageBox.warning(
            self.__mainWindow, caption, text, QMessageBox.Yes, QMessageBox.No
        )
