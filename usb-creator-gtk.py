...
parser.set_defaults(iso=None, allow_system_internal=False, trace=False)
# FIXME evand 2009-07-28: Reconnect this option to the install routine.
parser.add_option(
    "-i", "--iso", dest="img", help=_("provide a source image to pre-populate the UI.")
)
parser.add_option(
    "--allow-system-internal",
    dest="allow_system_internal",
    action="store_true",
...
try:
    ...
    frontend = GtkFrontend(
        backend, options.img, allow_system_internal=options.allow_system_internal
    )
